...
---@param dotEndNum number 小数点后几位数,默认0位，相当于取整
---@param type number 默认向下取整，1向上取整，2四舍五入
---@return number
function Mathf.FixedTo(num, dotEndNum, type)
    if num == nil or  num == 0 then
        return 0
    end
    if dotEndNum == nil then
        dotEndNum = 0
    end
    local fun
    if type == 1 then
    ...
    local str
    if dotEndNum == 0 then
        str = tostring(fun(num))
    else
        local powNum = Mathf.Pow(10, dotEndNum)
        num = num * powNum
        num = fun(num)
        num = num / powNum
        ---@type number
        local off = Mathf.Ceil(num)
        if Mathf.Abs(num - off) < 0.00001 then---不需要小数
            return off
        end
        return num
    end
...
---Round 四舍五入取 x 位小数
---@param target number
---@param num number 取几位小数
function Mathf.Round(target, num)
    if target == Mathf.Ceil(target) then
        return Mathf.Ceil(target)
    end
    local mulNum = Mathf.Pow(10, num)
    local target = target * mulNum

    local temp = Mathf.Ceil(target)
    if target + 0.5 >= temp then
        return temp / mulNum
    else
        return Mathf.Floor(target) / mulNum
    end
end
