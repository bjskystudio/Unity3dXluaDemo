Shader "EngineCenter/FX/FX_Base+Dissolve+Distortion" {
    Properties {
        [Header(BaseCol)]
        ...
        [MaterialToggle] _VertexColorOn ("VertexColorOn顶点颜色启用(粒子透明度可用）", Float ) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v)
            {
                ...
                float2 UVTime = float2(o.uv.r+_Time.r*_DistortionSpeed_XX,o.uv.g+_Time.r*_DistortionSpeed_YY);
                float4 DistortionTexRR_var = SAMPLE_TEXTURE2D_LOD(_DistortionTexRR, sampler_DistortionTexRR,TRANSFORM_TEX(UVTime, _DistortionTexRR), 0);
                float DistortionPow = saturate(DistortionTexRR_var.r*_DistortionPower);
                v.vertex.xyz += (DistortionPow*_DiatortionForVertexOffset*v.normal);
            ...
            float4 frag(VertexOutput i) : COLOR 
            {
                ...
                //distortion
                float2 DistortionSpeed = float2((i.uv.r+(_Time.r*_DistortionSpeed_XX)),(i.uv.g+(_Time.r*_DistortionSpeed_YY)));
                float4 DistortionTexRR_var = SAMPLE_TEXTURE2D(_DistortionTexRR,sampler_DistortionTexRR,TRANSFORM_TEX(DistortionSpeed, _DistortionTexRR));
                float DistortionTex = saturate(DistortionTexRR_var.r*_DistortionPower);                

                //MainTex 
                float MtRotator = _MainTexRotator * 6.284 / 360.0;
                float MtRotator_cos = cos(MtRotator);
                float MtRotator_sin = sin(MtRotator);
                float2 UVSpeed = lerp(float2(i.uv.x+(_Time.r*_MainTexSpeed_XX),(_Time.r*_MainTexSpeed_YY)+i.uv.y),float2(DistortionTex,DistortionTex),_Distortion);
                ...
                //MsakTex
                    ...
                    float2 MaskOrDistortion = lerp(i.uv,float2(DistortionTex, DistortionTex), lerp(0.0, _Distortion, _DistortionForMask));
                    float2 MaskUV =  mul(MaskOrDistortion - float2(0.5,0.5), float2x2(MaskRotator_cos, -MaskRotator_sin, MaskRotator_sin, MaskRotator_cos)) + float2(0.5,0.5);
                ...
                //detialtex
                    ...
                    float2 detailUVDistortion = detailUVSpeed +float2(DistortionTex, DistortionTex);
                    detailUVDistortion = lerp(detailUVSpeed , detailUVDistortion , lerp(0.0, 0.5, _DistortionForDetail));
                    float2 detailUV = mul(detailUVDistortion-float2(0.5,0.5),float2x2( DtRotator_cos, -DtRotator_sin, DtRotator_sin, DtRotator_cos))+float2(0.5,0.5);
                ...
                float2 DissSpeed = lerp(float2(i.uv.r+_Time.r*_DissolveSpeed_XX,i.uv.g+_Time.r*_DissolveSpeed_YY),float2(DistortionTex, DistortionTex), lerp(0.0, _Distortion, _DistortionForDissolve));
                ...
                #ifdef DEPTHRENDER
                    float Alpha = fade*(MainTex.a*lerp(1.0,i.vertexColor.a,_VertexColorOn)*_TintColor.a*MaskTex.a*DissolveOuterEdge);
                #else
                    float Alpha = MainTex.a * lerp(1.0, i.vertexColor.a, _VertexColorOn) * _TintColor.a*MaskTex.a*DissolveOuterEdge;
                 #endif
