Shader "Snk/Character/PBR/S_PBR_Character_Depth"
{
    Properties
    {
        ...
        _OffsetMul("_RimWidth", Range(0, 1)) = 1
        _Threshold("_Threshold", Range(0, 1)) = 1
        _FresnelMask("_FresnelMask", Range(0, 1)) = 1
        _Leftatten("_Leftatten", Range(0, 1)) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 DepthRimhLight(float3 viewPos, half3 worldPos,float4 positionNDC, half3 normalVS,half NdotV, half NdotL,float3 LightColor)
            {   
                float3 positionVS = viewPos; //物体的观察空间
                float3 offsetPositionVS = float3(positionVS.xy + normalVS.xy * lerp(0,0.01,_OffsetMul), positionVS.z); // 保持z不变（CS.w = -VS.z）
                float4 offsetPositionCS = TransformWViewToHClip(offsetPositionVS); // input.positionCS不是真正的CS 而是SV_Position屏幕坐标
                float4 offsetPositionVP = TransformHClipToViewPortPos(offsetPositionCS);
                float offsetDepth = SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, offsetPositionVP).r; // _CameraDepthTexture.r = input.positionNDC.z / input.positionNDC.w
                float depth = positionNDC.z / positionNDC.w;
                float linearEyeDepth = LinearEyeDepth(depth, _ZBufferParams); // 离相机越近越亮    
                float OffsetlinearEyeDepth = LinearEyeDepth(offsetDepth, _ZBufferParams);
                float depthDiff = OffsetlinearEyeDepth - linearEyeDepth;//深度相减得到边缘光
                float rimIntensity = step(lerp(0.1,0.2,_Threshold), depthDiff) ;
                float rimRatio = 1 - NdotV;
                rimRatio = pow(rimRatio, exp2(lerp(4.0, 0.0, _FresnelMask)));
                rimIntensity = lerp(0, rimIntensity, rimRatio);
                float3 RimIntensity = float3(rimIntensity,rimIntensity,rimIntensity) * _RimCol.rgb * saturate(pow((1-NdotL),_Leftatten*2)) * SampleSH(worldPos).rgb * LightColor;
                return RimIntensity;
            }
            ...
            float4 frag (v2f i) : SV_Target
            {
                    ...
                    half3 normalVS = TransformWorldToViewDir(normalize(i.worldNormal));
                    ...
                    float factor=step(_RimFadeVector.w,0)+saturate(dot(viewNormal.xz,_RimFadeVector.xy))*(1-clamp(0,1,_RimFadeVector.z));
                        ...
                        RimLight = DepthRimhLight(i.viewPos,i.worldPos,i.positionNDC,normalVS,NdotV,NdotL,LightColor) + DiffuseTeam; //(float3 viewPos, half3 worldPos,float4 positionNDC, half3 normalVS,half NdotV, half NdotL,float3 LightColor)
                    #endif
