Shader "Snk/Scene/Snk_Scene_Object"
{
    Properties
    {   
        ...
        [Header(OUTSIDELIGHT)]
        _OutsidelightToggsss("开关aaa",Float) = 0
        _RimColor("边缘颜色", Color) = (1,1,1,1)
        _RimPower("边缘强度",range(1,20)) = 2
        ...
        _RainRippleDisturbMat("_RainRippleDisturbMat", Range(0, 5)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
           ...
           //float _OutsidelightToggsss;
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                float3 outsideLight = pow((1-NdotV.x),_RimPower) * _RimColor.rgb;
                DiffuseTeam += lerp(0,outsideLight,_OutsidelightToggsss);
                ...
                //Ripple
                half diffuseFactor = 1;
                half roughness = 1;
                half wet = 0;
                #if _RAIN_ON
                    half puddleWater = saturate((_PuddleLevel - i.color.a) / 0.6);
                    half accumulateWater = puddleWater; //积水区域
                    accumulateWater = lerp(0, accumulateWater, i.color.r);
                    wet = saturate(_WetLevel + accumulateWater); //湿润区域
                    ...
                    half3 rippleNormal = 0;
                    RAIN_RIPPLE_NORMAL(rippleNormal, i.uv, _RainRippleIntensityMat, _RainRippleDisturbMat, _RainRippleDensityMat.xy);
                    half3 normalWS = normalize(lerp(normalize(i.worldNormal), rippleNormal, accumulateWater));
                    
                    float directDiffuseIntensity = lerp(1, 0.3, wet);
                    diffuseFactor = directDiffuseIntensity * (dot(LightDir, normalWS) * 0.5 + 0.5);
                #endif

                //漫反射最终颜色
                float3 finalColor = DiffuseTeam * diffuseFactor  + Emiss;

                //镜面反射
                #if _RAIN_ON
                    half mip = roughness * (1.7 - 0.7 * roughness) * 6;
                    float2 screenUV = i.screenPos.xy / i.screenPos.w;
                    float3 reflectCol = SAMPLE_TEXTURE2D_LOD(_ReflectionTex, sampler_ReflectionTex, screenUV, mip).rgb;
                    reflectCol *= saturate(dot(Viewdir, normalWS) * 0.5 + 0.5); 
                    finalColor = lerp(finalColor, lerp(finalColor, reflectCol, LimMap.b), _ReflectionIntensity);
                    finalColor = lerp(finalColor, reflectCol, saturate(_WetLevel * 0.1 + accumulateWater));
                #else
                    #ifdef REFLECTION 
                        float2 screenUV = i.screenPos.xy / i.screenPos.w;
                        float3 reflectCol = SAMPLE_TEXTURE2D(_ReflectionTex, sampler_ReflectionTex, screenUV).rgb;
        ...
        Pass
        {
            ...
            half4 DepthOnlyFragment(Varyings input) : SV_TARGET
            {
                ...
                float4 FixMask = SAMPLE_TEXTURE2D(_FixMask,sampler_FixMask, input.uv);             
                ...
                float4 albedo = SAMPLE_TEXTURE2D(_MainTex,sampler_MainTex, float2(input.uv.x + SpeedX, input.uv.y + SpeedY));
                float Alpha = albedo.a * _Color.a;
                //#ifdef ALPHACLIP
                    clip(Alpha - _CutOut);
                //#endif    
