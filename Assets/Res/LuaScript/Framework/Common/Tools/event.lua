...
local _pcall = {
    __call = function(self, ...--[[TODO:为了方便结果的对比，逻辑写在了一行中，确定后再优化代码的编码风格]]
    )
        --[[ local status, err
		...
		end				  -- [TODO:优化后的逻辑无法做到容错处理，是否是必做的？？？]
		return status ]]
        if (not self.obj) then
            self.func(...)
            return true
...
_event.__call = function(self, ...--[[TODO:为了方便结果的对比，逻辑写在了一行中，确定后再优化代码的编码风格]]
)
	...
	-- mark by solyess: all event handler maybe memory leaks.
    ...
    for k, v in pairs(self.opList) do
        op()
    end
end
...
BeforeUpdateBeat = event("BeforeUpdate")
