...
---@field black Color
---@field yellow Color
---@field cyan Color 蓝绿色
---@field magenta Color 玫瑰色
---@field gray Color 灰色
---@field clear Color 空白
---@field blackaplha Color 0.8黑色透明
---@field gamma Color 伽马射线
---@field linear Color 线性
---
local Color = {}
local _getter = {}
...
---通过传入一个16进制的色值创建一个颜色
---@param color16 number 0xffff00
---@param alpha number 透明度，0-255
---@return Color
function Color.CreateColor(color16, alpha)
    if alpha == nil then
        alpha = 255
    end
    local r,g,b
    ...
    b = color16 - (g << 8) - (r << 16)
    local per = 255
    return Color.New(r/per, g/per, b/per, alpha/per)
end
...
function Color.HSVToRGB(H, S, V, hdr)
    hdr = nil == hdr and true or hdr--x = a and b or c 当b不等于nil或false时,可以正常模拟
...
_getter.blackaplha = function()
    return Color.New(0, 0, 0, 0.8)
end
