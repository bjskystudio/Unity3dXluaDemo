...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class CLights
    {
        ...
        public CLights()
        {
            m_UseStructuredBuffer = CUtils.useStructuredBuffer;
            ...
            else
            {
	            ...
	            int maxLights = CNPRPipeline.maxVisibleAdditionalLights;
        ...
        public void Setup(ScriptableRenderContext context, ref SRenderingData renderingData)
        {
        ...
        void InitializeLightConstants(NativeArray<VisibleLight> lights, int lightIndex, out Vector4 lightPos, out Vector4 lightColor, out Vector4 lightAttenuation, out Vector4 lightSpotDir, out Vector4 lightOcclusionProbeChannel)
        {
            ...
            if (lightData.lightType != LightType.Directional)
            {
                // Light attenuation in NPR matches the unity vanilla one.
        ...
        void SetupShaderLightConstants(CommandBuffer cmd, ref SRenderingData renderingData)
        {
            m_MixedLightingSetup = MixedLightingSetup.None;

            // Main light has an optimized shader path for main light. This will benefit games that only care about a single light.
            // NPR pipeline also supports only a single shadow light, if available it will be the main light.
        ...
        void SetupAdditionalLightConstants(CommandBuffer cmd, ref SRenderingData renderingData)
        {
            ref LightData lightData = ref renderingData.lightData;
            var cullResults = renderingData.cullResults;
            var lights = lightData.visibleLights;
            int maxAdditionalLightsCount = CNPRPipeline.maxVisibleAdditionalLights;
            ...
            if (additionalLightsCount > 0)
            {
                if (m_UseStructuredBuffer)
                {
                    ...
                    var lightDataBuffer = CShaderData.instance.GetLightDataBuffer(additionalLightsCount);
                    lightDataBuffer.SetData(additionalLightsData);

                    int lightIndices = cullResults.lightAndReflectionProbeIndexCount;
                    var lightIndicesBuffer = CShaderData.instance.GetLightIndicesBuffer(lightIndices);
        ...
        int SetupPerObjectLightIndices(CullingResults cullResults, ref LightData lightData)
        {
            ...
            {
                if (additionalLightsCount >= CNPRPipeline.maxVisibleAdditionalLights)
                    break;
            ...
            if (m_UseStructuredBuffer && additionalLightsCount > 0)
            {
                ...
                cullResults.FillLightAndReflectionProbeIndices(CShaderData.instance.GetLightIndicesBuffer(lightAndReflectionProbeIndices));
            }
