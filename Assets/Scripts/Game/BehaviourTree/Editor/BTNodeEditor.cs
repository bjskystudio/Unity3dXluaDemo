...
namespace LuaBehaviourTree
{
    ...
    public class BTNodeEditor : EditorWindow
    {
        ...
        [MenuItem("Tools/AI/行为树编辑器")]
        public static BTNodeEditor ShowEditor()
        {
        ...
        private Rect GetNodeRect(Vector2 pos)
        {
            var rect = new Rect
            {
                x = pos.x,
                y = pos.y,
                width = NodeWindowWidth,
                height = NodeWindowHeight
            };
            return rect;
        }
        ...
        private void OnEnable()
        {
            ...
            {
                { EBTNodeType.EntryNodeType, Color.white },
                { EBTNodeType.ActionNodeType, Color.red },
                { EBTNodeType.CompositeNodeType, Color.blue },
                { EBTNodeType.ConditionNodeType, Color.yellow },
                { EBTNodeType.DecorationNodeType, Color.cyan }
            };
            mNodeTypeNameMap = new Dictionary<EBTNodeType, string>
            {
                { EBTNodeType.EntryNodeType, "根节点" },
                { EBTNodeType.ActionNodeType, "行为节点" },
                { EBTNodeType.CompositeNodeType, "组合节点" },
                { EBTNodeType.ConditionNodeType, "条件节点" },
                { EBTNodeType.DecorationNodeType, "装饰节点" }
            };
        ...
        private void ReadAllNodesUID()
        {
            ...
            var saveFolderFullPath = $"Assets/Res/{BTData.BTNodeSaveFolderRelativePath}/";
            var allFilePaths = YKEditorUtils.GetFilesPaths(saveFolderFullPath, "*.asset");
            foreach (var path in allFilePaths)
            {
                var btgraph = AssetDatabase.LoadAssetAtPath<BTGraph>(path);
                var fileName = path.Replace(saveFolderFullPath, "").Replace(".asset", "");
                if (fileName != btgraph.BTFileName)
                {
                    Debug.LogError($"文件相对路径记录有误，原{btgraph.BTFileName}，修正为{fileName},路径:{path}");
                    btgraph.BTFileName = fileName;
            ...
            sw.Stop();
            Debug.Log($"读取的行为树文件数量:{allFilePaths.Length} 已使用UID数量:{AllUsedNodeUIDMap.Count}");
        ...
        private void OnSelectionChange()
        {
            ...
            {
                if (mCurrentSelectionBTGraph != null && !string.IsNullOrEmpty(mCurrentSelectionBTGraphAssetPath))
                {
                    EditorUtility.SetDirty(mCurrentSelectionBTGraph);
                    AssetDatabase.SaveAssets();
                }
                var selectionAsset = Selection.objects[0];
                if (selectionAsset is BTGraph)
                {
                    LoadBTAsset(selectionAsset as BTGraph);
                }
                else if (selectionAsset is GameObject)
                {
                    ...
                    if (tbt != null && tbt.BTOriginalGraph != null)
                    {
                        if (tbt.BTRunningGraph != null)
                        {
                            mCurrentSelectionBTGraph = tbt.BTRunningGraph;
                            mCurrentSelectionBTGraphAssetOriginalName = tbt.BTOriginalGraph.BTFileName;
                            mCurrentSelectionBTGraphAssetPath = AssetDatabase.GetAssetPath(tbt.BTOriginalGraph);
                ...
                {
                    //Debug.Log($"选中的是非BTGraph也非有效TBehaviourTree，不处理!");
                    return;
                }
            }
        }
        ...
        public void LoadBTAsset(BTGraph asset)
        {
            var assetpath = AssetDatabase.GetAssetPath(asset);
            ReadAllNodesUID();
            mCurrentSelectionBTGraph = asset;
            mCurrentSelectionBTGraph.InitOtherData();
            mCurrentSelectionBTGraph.InitNoteData();
            mCurrentSelectionBTGraphAssetOriginalName = asset.BTFileName;
        ...
        private void OnGUI()
        {
            ...
            else
            {
                ...
                {
                    EditorGUILayout.LabelField("运行时请选择拥有AI行为树的对象!", mLableAlignMiddleStyle, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
                }
                else
                {
                    if (GUILayout.Button("新建", GUILayout.ExpandWidth(true), GUILayout.Height(60.0f)))
                    {
                        CreateNewBTAsset();
                    }
        ...
        private void DrawSelectTollBarInspector()
        {
            ...
            if (!string.IsNullOrEmpty(mCurrentSelectionBTGraphAssetPath))
            {
        ...
        private void DrawUserOperationPanel()
        {
            ...
            if (IsDebugMode && Application.isPlaying && mCurrentSelectionBTGraph.ExecutingNodesMap != null)
            {
        ...
        private void DrawParamsPanel()
        {
            ...
            {
                EditorGUILayout.BeginVertical("box", GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
                EditorUtilities.DisplayDIYGUILable("参数面板", Color.yellow, 0, ToolBarWidth - 10, 20.0f);
                EditorGUILayout.BeginHorizontal();
                ...
                mCurrentClickNode.NodeParams = EditorGUILayout.TextField(mCurrentClickNode.NodeParams, GUILayout.Width(ToolBarWidth - 10), GUILayout.Height(20.0f));
        ...
        private void DrawVariablesPanel()
        {
            if (!string.IsNullOrEmpty(mCurrentSelectionBTGraphAssetPath))
            {
                ...
                EditorGUILayout.BeginVertical("box", GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
                EditorUtilities.DisplayDIYGUILable("变量面板", Color.yellow, 0, ToolBarWidth - 10, 20.0f);
                EditorGUILayout.BeginHorizontal();
        ...
        /// </summary>
        /// <returns></returns>
        public bool TrySaveBTAsset()
        {
            if (/*Application.isPlaying == false*/true)
            {
                if (string.IsNullOrEmpty(mCurrentSelectionBTGraph.BTFileName) == false)
                {
                    if (mCurrentSelectionBTGraph.BTFileName == DefaultBTGrapshFileName)
                    {
                        Debug.LogError("请修改行为树的命名!");
                        return false;
                    }
                    // 检查行为树的有效性
                    if (IsValideTree())
                    {
                        ...
                        if (!string.IsNullOrEmpty(mCurrentSelectionBTGraphAssetOriginalName) &&
                            !mCurrentSelectionBTGraph.BTFileName.Equals(mCurrentSelectionBTGraphAssetOriginalName) &&
                            !string.IsNullOrEmpty(mCurrentSelectionBTGraphAssetPath))
                        {
                            if (EditorUtility.DisplayDialog("提示", $"文件名从:{mCurrentSelectionBTGraphAssetOriginalName}变到{mCurrentSelectionBTGraph.BTFileName}", "确定", "取消"))
                            {
                                //文件名有变化，改名
                                Debug.Log($"文件名从:{mCurrentSelectionBTGraphAssetOriginalName}变到{mCurrentSelectionBTGraph.BTFileName},老文件:{mCurrentSelectionBTGraphAssetPath}!");
                                AssetDatabase.RenameAsset(mCurrentSelectionBTGraphAssetPath, mCurrentSelectionBTGraph.BTFileName);
                            }
                        }
                        else
                        {
                            var asset = AssetDatabase.LoadAssetAtPath<BTGraph>(mCurrentSelectionBTGraphAssetPath);
                            ...
                            {
                                //Asset不存在
                                var defaultsavefolderfullpath = $"{Application.dataPath}/Res/{BTData.BTNodeSaveFolderRelativePath}";
                                AorIO.CreateDir(defaultsavefolderfullpath);
                                var savefolderpath = EditorUtility.OpenFolderPanel("选择保存目录", defaultsavefolderfullpath, "");
                                if (!string.IsNullOrEmpty(savefolderpath))
                                {
                                    assetsavefullpath = $"{savefolderpath.Replace(Application.dataPath, "Assets")}/{mCurrentSelectionBTGraph.BTFileName}.asset";
                                    AssetDatabase.CreateAsset(mCurrentSelectionBTGraph, assetsavefullpath);
                                    Debug.Log($"保存成功:{assetsavefullpath}");
                                }
                                else
                                {
                        ...
                        //mCurrentSelectionBTGraphAsset = AssetDatabase.LoadAssetAtPath<BTGraph>(newbtgraphassetpath);
                        //LoadBTAsset(mCurrentSelectionBTGraphAsset);
                        LoadBTAsset(mCurrentSelectionBTGraph);
                        return true;
                    }
                    else
                    {
            ...
            if (mCurrentSelectionBTGraph != null)
            {
                foreach (var node in mCurrentSelectionBTGraph.AllNodesList)
                {
                    if (node.AbortType == 0)
                    {
                        ...
                        Debug.Log($"矫正节点UID:{node.UID}的AbortType:{node.AbortType}");
                    }
        ...
        private void RecaculateAllNodesUID()
        {
            if (Application.isPlaying == false)
            {
                if (mCurrentSelectionBTGraphAssetOriginalName != DefaultBTGrapshFileName)
                {
        ...
        private void CreateNewBTAsset()
        {
            ...
            if (mCurrentSelectionBTGraph != null)
            {
                mCurrentSelectionBTGraph = null;
            }
            mCurrentSelectionBTGraphAssetOriginalName = DefaultBTGrapshFileName;
            mCurrentSelectionBTGraph = ScriptableObject.CreateInstance<BTGraph>();
            mCurrentSelectionBTGraph.Init(DefaultBTGrapshFileName);
        ...
        {
            for (int i = 0; i < BTNodeData.BTLuaActionNodeNameArray.Length; i++)
            {
                var actionnodename = BTNodeData.BTLuaActionNodeNameArray[i];
                var nodeinfo = new CreateNodeInfo(mCurrentSelectionBTGraph, operationnode, actionnodename);
                menu.AddItem(new GUIContent($"添加子节点/行为节点/{BTNodeData.BTLuaActionNodeIntroArray[i]}"), false, OnCreateBTActionNode, nodeinfo);
            }
            //foreach (var actionnodename in BTNodeData.BTLuaActionNodeNameArray)
            //{
            //    var nodeinfo = new CreateNodeInfo(mCurrentSelectionBTGraph, operationnode, actionnodename);
            //    menu.AddItem(new GUIContent($"添加子节点/行为节点/{actionnodename}"), false, OnCreateBTActionNode, nodeinfo);
            //}
            for (int i = 0; i < BTNodeData.BTCSActionNodeNameArray.Length; i++)
            {
                var csactionnodename = BTNodeData.BTCSActionNodeNameArray[i];
                var nodeinfo = new CreateNodeInfo(mCurrentSelectionBTGraph, operationnode, csactionnodename);
                menu.AddItem(new GUIContent($"添加子节点/行为节点/{BTNodeData.BTCSActionNodeIntroArray[i]}"), false, OnCreateBTActionNode, nodeinfo);
            }
            ...
            //}
            for (int i = 0; i < BTNodeData.BTCompositeNodeNameArray.Length; i++)
            {
                var nodeinfo = new CreateNodeInfo(mCurrentSelectionBTGraph, operationnode, BTNodeData.BTCompositeNodeNameArray[i]);
                menu.AddItem(new GUIContent($"添加子节点/组合节点/{BTNodeData.BTCompositeNodeIntroArray[i]}"), false, OnCreateBTCompositeNode, nodeinfo);
            }
            ...
            //}
            for (int i = 0; i < BTNodeData.BTLuaConditionNodeNameArray.Length; i++)
            {
                var nodeinfo = new CreateNodeInfo(mCurrentSelectionBTGraph, operationnode, BTNodeData.BTLuaConditionNodeNameArray[i]);
                menu.AddItem(new GUIContent($"添加子节点/条件节点/{ BTNodeData.BTLuaConditionNodeIntroArray[i]}"), false, OnCreateBTConditionNode, nodeinfo);
            }
            //foreach (var conditionnodename in BTNodeData.BTLuaConditionNodeNameArray)
            //{
            ...
            //}
            for (int i = 0; i < BTNodeData.BTCSConditionNodeNameArray.Length; i++)
            {
                var nodeinfo = new CreateNodeInfo(mCurrentSelectionBTGraph, operationnode, BTNodeData.BTCSConditionNodeNameArray[i]);
                menu.AddItem(new GUIContent($"添加子节点/条件节点/{BTNodeData.BTCSConditionNodeIntroArray[i]}"), false, OnCreateBTConditionNode, nodeinfo);
            }
            ...
            //}
            for (int i = 0; i < BTNodeData.BTDecorationNodeNameArray.Length; i++)
            {
                var nodeinfo = new CreateNodeInfo(mCurrentSelectionBTGraph, operationnode, BTNodeData.BTDecorationNodeNameArray[i]);
                menu.AddItem(new GUIContent($"添加子节点/装饰节点/{BTNodeData.BTDecorationNodeIntroArray[i]}"), false, OnCreateBTDecorationNode, nodeinfo);
            }
        ...
        private void AddAllAvalibleReplaceBTNodeMenu(GenericMenu menu, BTNode operationnode)
        {
            if (operationnode != null)
            {
                ...
                List<string> validereplacenodenamelist2 = new List<string>();
                if (operationnode.NodeType == (int)EBTNodeType.ActionNodeType)
                {
                    ...
                    validereplacenodenamelist2.AddRange(BTNodeData.BTLuaActionNodeIntroArray);
                    validereplacenodenamelist2.AddRange(BTNodeData.BTCSActionNodeIntroArray);
                }
                else if (operationnode.NodeType == (int)EBTNodeType.CompositeNodeType)
                {
                    ...
                    validereplacenodenamelist2.AddRange(BTNodeData.BTCompositeNodeIntroArray);
                }
                else if (operationnode.NodeType == (int)EBTNodeType.ConditionNodeType)
                {
                    ...
                    validereplacenodenamelist2.AddRange(BTNodeData.BTLuaConditionNodeIntroArray);
                    validereplacenodenamelist2.AddRange(BTNodeData.BTCSConditionNodeIntroArray);
                }
                else if (operationnode.NodeType == (int)EBTNodeType.DecorationNodeType)
                {
                    ...
                    validereplacenodenamelist2.AddRange(BTNodeData.BTDecorationNodeIntroArray);
                }
                if (string.IsNullOrEmpty(validereplacetypename) == false)
                {
                    for (int i = 0; i < validereplacenodenamelist.Count; i++)
                    {
                        var nodeinfo = new CreateNodeInfo(mCurrentSelectionBTGraph, operationnode, validereplacenodenamelist[i]);
                        menu.AddItem(new GUIContent($"替换/{validereplacetypename}/{validereplacenodenamelist2[i]}"), false, OnReplaceBTNode, nodeinfo);
                    }
