...
namespace LuaBehaviourTree
{
    ...
    **/
    ...
    /// 鼠标右击会显示该菜单
    /// </summary>
    [CreateAssetMenu(fileName = "NewAICfg", menuName = "AI/新建AI行为树配置")]
    ...
    public class BTGraph : ScriptableObject
    {
        ...
        /// 所有String变量节点定义数据列表
        /// </summary>
        public List<CustomStringVariableNodeData> AllStringVariableNodeDataList = new List<CustomStringVariableNodeData>();
        ...
        public void Init(string btFileName = null)
        {
        ...
        /// <summary>
        /// 初始化节点数据(反序列化构建后必须调用确保RootNode指向正确对象)
        /// </summary>
        ...
        public void SetBTOwner(TBehaviourTree btowner)
        {
            ...
            AllBoolVariableDataList = btowner.BTOriginalGraph.AllBoolVariableDataList.Clone();
            AllIntVariableDataList = btowner.BTOriginalGraph.AllIntVariableDataList.Clone();
            AllFloatVariableDataList = btowner.BTOriginalGraph.AllFloatVariableDataList.Clone();
            AllStringVariableDataList = btowner.BTOriginalGraph.AllStringVariableDataList.Clone();
            AllBoolVariableNodeDataList = btowner.BTOriginalGraph.AllBoolVariableNodeDataList.Clone();
            AllIntVariableNodeDataList = btowner.BTOriginalGraph.AllIntVariableNodeDataList.Clone();
            AllFloatVariableNodeDataList = btowner.BTOriginalGraph.AllFloatVariableNodeDataList.Clone();
            AllStringVariableNodeDataList = btowner.BTOriginalGraph.AllStringVariableNodeDataList.Clone();
            OwnerBT = btowner;
            InitBlackBoard();
            InitCustomVariableNodeData();
            RootNodeUID = btowner.BTOriginalGraph.RootNodeUID;
            RootNode = BTUtilities.CreateRunningNodeByNode(btowner.BTOriginalGraph.RootNode, btowner, null, btowner.WorldItemUID, btowner.InstanceID);
        }
        ...
        private void InitCustomVariableNodeData()
        {
            ...
            {
                //Log.Info($"节点UID:{boolvariablenodedata.NodeUID}自定义变量值:{(boolvariablenodedata as CustomBoolVariableNodeData).VariableValue}");
            ...
            {
                //Log.Info($"节点UID:{intvariablenodedata.NodeUID}自定义变量值:{(intvariablenodedata as CustomIntVariableNodeData).VariableValue}");
            ...
            {
                //Log.Info($"节点UID:{floatvariablenodedata.NodeUID}自定义变量值:{(floatvariablenodedata as CustomFloatVariableNodeData).VariableValue}");
            ...
            {
                //Log.Info($"节点UID:{stringvariablenodedata.NodeUID}自定义变量值:{(stringvariablenodedata as CustomStringVariableNodeData).VariableValue}");
        ...
        public void Dispose()
        {
            ...
            if (OwnerBT != null)
            {
                ...
                for (int i = 0, length = AllNodesList != null ? AllNodesList.Count : 0; i < length; i++)
                {
                    ...
                    ObjectPool.Instance.PushAsObj(AllNodesList[i]);
                }
            }
            AllNodesList.Clear();
            RootNode = null;
        ...
        public bool AddExecutingNode(BTNode node)
        {
            if (node != null)
            {
                ...
                {
                    //Log.Info($"添加UID:{node.UID}的执行节点!");
                    ExecutingNodesMap.Add(node.UID, node);
                    return true;
                }
                else
                {
                    //Log.Error($"重复添加UID:{node.UID}的执行节点!");
                    return false;
                }
            }
            else
            {
                Log.Error("不允许添加空的执行节点!");
                return false;
            }
        }
        ...
        public bool UpdateExecutedConditionNodeResult(BTNode node, EBTNodeRunningState result)
        {
            if (node != null)
            {
                ...
                {
                    //Log.Info($"添加UID:{node.UID}的已执行条件节点执行结果{result}!");
                    ExecutedReevaluatedNodesResultMap.Add(node, result);
                    return true;
                }
                else
                {
                    //Log.Info($"更新UID:{node.UID}的已执行条件节点执行结果:{result}!");
                    return true;
                }
            }
            else
            {
                Log.Error("不允许更新空的已执行条件节点结果!");
                return false;
            }
        }
        ...
        protected bool ReevaluatedExecutedNodes()
        {
            if (ExecutedReevaluatedNodesResultMap.Count > 0)
            {
                ...
                foreach (var conditionnode in allkeys)
                {
                    ...
                    {
                        //Log.Info($"节点UID:{conditionnode.UID}的条件节点状态由:{ExecutedReevaluatedNodesResultMap[conditionnode]}变到{newresult},需要打断行为树重置整棵树运行!");
                        return true;
                    }
                }
                return false;
        ...
        public CustomVariableNodeData GetVariableNodeValueInRuntime(int uid)
        {
            ...
            {
                Log.Error($"找不到的UID:{uid}自定义变量节点数据,获取节点数据失败!");
                return null;
            }
        }
        #endregion
        ...
        public bool AddNode(BTNode node)
        {
            ...
            {
                Log.Info($"不允许添加空或UID:{node.UID}的重复节点!");
                return false;
            }
        }
        #endregion
        ...
        public CustomVariableData GetVariableDefaultValueInEditor(string variablename, EVariableType variabletype)
        {
            switch (variabletype)
            {
                case EVariableType.Bool:
                    return new CustomBoolVariableData(variablename, variabletype, default);
                case EVariableType.String:
                    return new CustomStringVariableData(variablename, variabletype, default);
                case EVariableType.Float:
                    return new CustomFloatVariableData(variablename, variabletype, default);
                case EVariableType.Int:
                    return new CustomIntVariableData(variablename, variabletype, default);
                default:
                    Log.Error($"不支持的变量类型:{variabletype},获取默认值失败!");
                    return null;
            }
        }
        ...
        public string[] GetCustomVariableNames(EVariableType variabletype)
        {
            ...
            switch (variabletype)
            {
                ...
                case EVariableType.Int:
                    ...
                    break;
                default:
                    Log.Error($"不支持添加的自定义变量类型:{variabletype},获取选项失败!");
                    return null;
            }
            // 默认第一个是空选项
        ...
        {
            if (customvariabledata == null)
            {
                return;
            }
            switch (customvariabledata.VariableType)
            {
                case EVariableType.Bool:
                    AllBoolVariableDataList.Add(customvariabledata as CustomBoolVariableData);
                    Log.Info($"添加变量名:{customvariabledata.VariableName}Bool类型自定义变量节点数据!");
                    break;
                case EVariableType.String:
                    AllStringVariableDataList.Add(customvariabledata as CustomStringVariableData);
                    Log.Info($"添加变量名:{customvariabledata.VariableName}String类型自定义变量节点数据!");
                    break;
                case EVariableType.Float:
                    AllFloatVariableDataList.Add(customvariabledata as CustomFloatVariableData);
                    Log.Info($"添加变量名:{customvariabledata.VariableName}Float类型自定义变量节点数据!");
                    break;
                case EVariableType.Int:
                    AllIntVariableDataList.Add(customvariabledata as CustomIntVariableData);
                    Log.Info($"添加变量名:{customvariabledata.VariableName}Int类型自定义变量节点数据!");
                    break;
                default:
                    break;
            }
        ...
        public void AddCustomVariableNodeData(CustomVariableNodeData customvariablenodedata)
        {
            if (customvariablenodedata.VariableType == EVariableType.Bool)
            {
                ...
                Log.Info($"添加节点UID:{customvariablenodedata.NodeUID} Bool类型自定义变量节点数据!");
            }
            else if (customvariablenodedata.VariableType == EVariableType.Int)
            {
                ...
                Log.Info($"添加节点UID:{customvariablenodedata.NodeUID} Int类型自定义变量节点数据!");
            }
            else if (customvariablenodedata.VariableType == EVariableType.Float)
            {
                ...
                Log.Info($"添加节点UID:{customvariablenodedata.NodeUID} Floatl类型自定义变量节点数据!");
            }
            else if (customvariablenodedata.VariableType == EVariableType.String)
            {
                ...
                Log.Info($"添加节点UID:{customvariablenodedata.NodeUID} String类型自定义变量节点数据!");
            }
            else
            {
                Log.Error($"不支持添加的自定义变量类型:{customvariablenodedata.VariableType}的节点数据,添加失败!");
            }
        }
        ...
        public bool MoveNodeBackward(BTNode childnode)
        {
            ...
            {
                Debug.LogWarning($"节点名:{parentnode.NodeName}的UID:{childnode.UID}子节点已经是最后一个子节点,无法再向后移动!");
                return false;
            }
        }
        ...
        private bool CheckVariableNodeDataValidation()
        {
            ...
            {
                Log.Error($"节点UID:{variablenodedata.NodeUID}自定义变量未设置有效变量名:{variablenodedata.VariableName}!");
                return false;
            }
            else
            {
                foreach (var boolvariablendoedata in AllBoolVariableNodeDataList)
                {
                    ...
                    {
                        Log.Error($"节点UID:{boolvariablendoedata.NodeUID}自定义变量:{boolvariablendoedata.VariableName}已经不存在了!");
                        return false;
                    }
                }
            }
            ...
            else
            {
                foreach (var intvariablendoedata in AllIntVariableNodeDataList)
                {
                    ...
                    if (variabledata == null)
                    {
                        Log.Error($"节点UID:{intvariablendoedata.NodeUID}自定义变量:{intvariablendoedata.VariableName}已经不存在了!");
                        return false;
                    }
                }
            }
            ...
            else
            {
                foreach (var floatvariablendoedata in AllFloatVariableNodeDataList)
                {
                    ...
                    if (variabledata == null)
                    {
                        Log.Error($"节点UID:{floatvariablendoedata.NodeUID}自定义变量:{floatvariablendoedata.VariableName}已经不存在了!");
                        return false;
                    }
                }
            }
            ...
            else
            {
                foreach (var stringvariablendoedata in AllStringVariableNodeDataList)
                {
                    ...
                    if (variabledata == null)
                    {
                        Log.Error($"节点UID:{stringvariablendoedata.NodeUID}自定义变量:{stringvariablendoedata.VariableName}已经不存在了!");
                        return false;
                    }
                }
            }
