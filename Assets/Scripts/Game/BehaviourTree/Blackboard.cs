...
namespace LuaBehaviourTree
{
    ...
    public class Blackboard
    {
        ...
        public bool AddData(string key, IBlackboardData data)
        {
            ...
            {
                Log.Error(string.Format("黑板数据里已存在Key:{0}的数据，添加数据失败!", key));
                return false;
            }
        }
        ...
        public T GetData<T>(string key)
        {
            ...
            {
                Log.Error($"黑板里找不到数据Key:{key},返回默认值!");
                return default(T);
            }
        }
        ...
        public bool UpdateData<T>(string key, T data)
        {
            ...
            {
                Log.Error(string.Format("更新Key:{0}的数据失败!", key));
                return false;
            }
        }
        ...
        public void PrintAllBlackBoardDatas()
        {
            ...
            foreach (var blackboarddata in mBlackboardDataMap)
            {
                ...
                if (blackboarddatatype == typeof(BlackboardData<bool>))
                {
                    ...
                    Log.Info($"变量名:{blackboarddata.Key}变量值:{realblackboarddata.Data}");
                }
                ...
                {
                    Log.Error($"不支持的黑板数据类型:{blackboarddatatype},打印失败!");
                }
            }
        }
        ...
        {
            if (!mBlackboardDataMap.TryGetValue(key, out IBlackboardData value))
            {
                Log.Error(string.Format("找不到Key:{0}的黑板数据!", key));
            }
            return value;
        }
    }
