...
{
    [CustomEditorForRenderPipeline(typeof(Camera), typeof(CNPRPipelineAsset))]
    [CanEditMultipleObjects]
    class CNPRPipelineCameraEditor : CameraEditor
    {
        ...
        internal class Styles
        {
            ...
            //public static GUIContent cameraIdentity = EditorGUIUtility.TrTextContent("Identity", "Identity of camera this is.");
            public static GUIContent cameraType = EditorGUIUtility.TrTextContent("Type", "Controls which type of camera this is.");
            ...
            public static GUIContent clearDepthForDepthOnlyPass = EditorGUIUtility.TrTextContent("Clear Depth For DOP", "If enabled, depth from the previous DepthOnlyPass will be cleared.");
            ...
            public static GUIContent renderBlurProcessing = EditorGUIUtility.TrTextContent("Blur Processing", "Enable this to make this camera render blur processing effects.");
            public static GUIContent renderBloomProcessing = EditorGUIUtility.TrTextContent("Bloom Processing", "Enable this to make this camera render bloom processing effects.");
            ...
            public static GUIContent renderPostProcessingCulling = EditorGUIUtility.TrTextContent("Culling Mask", "Enable this to make this camera render post-processing effects.");
            ...
            public static readonly string hdrDisabledWarning = "HDR rendering is disabled in the NPR Pipeline asset.";
            public static readonly string mssaDisabledWarning = "Anti-aliasing is disabled in the NPR Pipeline asset.";

            public static readonly string missingRendererWarning = "The currently selected Renderer is missing form the NPR Pipeline asset.";
            public static readonly string noRendererError = "There are no valid Renderers available on the NPR Pipeline asset.";
        ...
        CNPRPipelineAsset m_NPRPipeline;
        CAdditionalCameraData m_AdditionalCameraData;
        ...
        //SerializedProperty m_AdditionalCameraDataCameraIdentityProp;
        ...
        SerializedProperty m_AdditionalCameraDataRenderBlurProcessing;
        SerializedProperty m_AdditionalCameraDataRenderBloomProcessing;
        SerializedProperty m_AdditionalCameraDataRenderPostProcessing;
        SerializedProperty m_AdditionalCameraDataRenderPostProcessingCulling;
        ...
        SerializedProperty m_AdditionalCameraClearDepthForDepthOnlyPass;
        ...
        {
            m_NPRPipeline = GraphicsSettings.renderPipelineAsset as CNPRPipelineAsset;
            ...
            m_AdditionalCameraData = camera.gameObject.GetComponent<CAdditionalCameraData>();
            ...
            {
                m_AdditionalCameraData = camera.gameObject.AddComponent<CAdditionalCameraData>();
            }
        ...
        void UpdateCameras()
        {
            var o = new PropertyFetcher<CAdditionalCameraData>(m_AdditionalCameraDataSO);
        ...
        void DrawElementCallback(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (cam != null)
            {
                bool warning = false;
                string warningInfo = "";
                var type = cam.gameObject.GetComponent<CAdditionalCameraData>().renderType;
                ...
                if (warning)
                {
                    ...
                    EditorGUI.LabelField(rect, m_NameContent, TempContent(type.ToString()/*.GetName()*/, warningInfo, m_ErrorIcon), errorStyle);
                }
                else
                {
                    ...
                    var isPostActive = cam.gameObject.GetComponent<CAdditionalCameraData>().renderPostProcessing;
                    if (isPostActive)
                    {
        ...
        void AddCameraToCameraList(Rect rect, ReorderableList list)
        {
            ...
            {
                var component = camera.gameObject.GetComponent<CAdditionalCameraData>();
                if (component != null)
                {
        ...
        void init(CAdditionalCameraData additionalCameraData)
        {
            ...
            m_AdditionalCameraDataRenderBlurProcessing = m_AdditionalCameraDataSO.FindProperty("m_bRenderBlurProcessing");
            m_AdditionalCameraDataRenderBloomProcessing = m_AdditionalCameraDataSO.FindProperty("m_bRenderBloomProcessing");
            m_AdditionalCameraDataRenderPostProcessing = m_AdditionalCameraDataSO.FindProperty("m_RenderPostProcessing");
            m_AdditionalCameraDataRenderPostProcessingCulling = m_AdditionalCameraDataSO.FindProperty("m_RenderPostProcessingCulling");
            ...
            m_AdditionalCameraClearDepthForDepthOnlyPass = m_AdditionalCameraDataSO.FindProperty("m_ClearDepthForDepthOnlyPass");
            //m_AdditionalCameraDataCameraIdentityProp = m_AdditionalCameraDataSO.FindProperty("m_iCameraIdentity");
            ...
            //CEditorUtils.CreateLayerInfo(out m_asLayerNames, out m_aiLayerValues);
        ...
        BackgroundType GetBackgroundType(CameraClearFlags clearFlags)
        {
            switch (clearFlags)
            {
                ...
                // DepthOnly is not supported by design in NPR RP. We upgrade it to SolidColor
                default:
        ...
        {
            if(m_NPRPipeline == null)
			{
				EditorGUILayout.HelpBox("NPR RP asset not assigned, assign one in the Graphics Settings.", MessageType.Error);
                return;
			}
            ...
            //EditorGUILayout.IntPopup(m_AdditionalCameraDataCameraIdentityProp, m_aLayerNames, m_aiLayerValues, Styles.cameraIdentity);
        ...
        void DrawStackSettings()
        {
            ...
            AScriptableRenderer.RenderingFeatures supportedRenderingFeatures = m_AdditionalCameraData?.scriptableRenderer?.supportedRenderingFeatures;
...
//#endif
            ...
            if (m_NPRPipeline.postProcessingFeatureSet == PostProcessingFeatureSet.PostProcessingV2)
            {
        ...
        void DrawRenderingSettings(CameraRenderType camType)
        {
            ...
            if (m_RenderingSettingsFoldout.value)
            {
                ...
                EditorGUILayout.PropertyField(m_AdditionalCameraDataRenderBlurProcessing, Styles.renderBlurProcessing);
                EditorGUILayout.PropertyField(m_AdditionalCameraDataRenderBloomProcessing, Styles.renderBloomProcessing);
                if (m_AdditionalCameraDataRenderBloomProcessing.boolValue && !CNPRPipeline.asset.cameraBloomEnable)
                    EditorGUILayout.HelpBox("The camera bloom function cannot be activated. because it is not activated in the current pipeline setting.", MessageType.Warning, true);
                ...
                else if (camType == CameraRenderType.Overlay)
                {
                    ...
                    EditorGUILayout.PropertyField(m_AdditionalCameraClearDepthForDepthOnlyPass, Styles.clearDepthForDepthOnlyPass);
                    m_AdditionalCameraDataSO.ApplyModifiedProperties();
                }
        ...
        void DrawPostProcessingOverlay()
        {
            ...
            if (selectedRenderPostProcessing)
            {
                EditorGUI.indentLevel++;
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(m_AdditionalCameraDataRenderPostProcessingCulling, Styles.renderPostProcessingCulling);
                if (EditorGUI.EndChangeCheck())
                    m_AdditionalCameraDataSO.ApplyModifiedProperties();
                EditorGUI.indentLevel--;
            }
            ...
            if (hasChanged)
            {
                if (m_AdditionalCameraDataSO == null)
                {
                    m_AdditionalCameraData = Undo.AddComponent<CAdditionalCameraData>(camera.gameObject);
                    init(m_AdditionalCameraData);
                }
        ...
        void DrawTargetTexture()
        {
            ...
            if (!settings.targetTexture.hasMultipleDifferentValues && m_NPRPipeline != null)
            {
                var texture = settings.targetTexture.objectReferenceValue as RenderTexture;
                int pipelineSamplesCount = m_NPRPipeline.msaaSampleCount;
                ...
                if (texture && texture.antiAliasing > pipelineSamplesCount)
                {
                    string pipelineMSAACaps = (pipelineSamplesCount > 1)
                        ...
                        : "has MSAA disabled";
                    EditorGUILayout.HelpBox(String.Format("Camera target texture requires {0}x MSAA. NPR pipeline {1}.", texture.antiAliasing, pipelineMSAACaps),
        ...
        void DrawRenderer()
        {
            ...
            int selectedRenderer = EditorGUILayout.IntPopup(Styles.rendererType, selectedRendererOption, m_NPRPipeline.rendererDisplayList, CNPRPipeline.asset.rendererIndexList);
            if (!m_NPRPipeline.ValidateRendererDataList())
            {
                EditorGUILayout.HelpBox(Styles.noRendererError, MessageType.Error);
            }
            else if (!m_NPRPipeline.ValidateRendererData(selectedRendererOption))
            {
        ...
        void DrawPostProcessing()
        {
            ...
            if (selectedRenderPostProcessing)
            {
                ...
                if (CNPRPipeline.asset?.postProcessingFeatureSet != PostProcessingFeatureSet.PostProcessingV2)
                {
    ...
    [ScriptableRenderPipelineExtension(typeof(CNPRPipelineAsset))]
    class NPRPipelineCameraContextualMenu : IRemoveAdditionalDataContextualMenu<Camera>
    {
        ...
        public void RemoveComponent(Camera camera, IEnumerable<Component> dependencies)
        {
            ...
            dependencies = dependencies.Where(c => c.GetType() != typeof(CAdditionalCameraData));
            ...
            Undo.SetCurrentGroupName("Remove NPR Camera");
            var additionalCameraData = camera.GetComponent<CAdditionalCameraData>();
