...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    /// </summary>
    public class CAdditionalLightsShadowCasterPass : AScriptableRenderPass
    {
        ...
        public CAdditionalLightsShadowCasterPass(RenderPassEvent evt)
        {
            renderPassEvent = evt;
        ...
        public bool Setup(ref SRenderingData renderingData)
        {
            Clear();
            ...
            for (int i = 0; i < visibleLights.Length && m_AdditionalShadowCastingLightIndices.Count < additionalLightsCount; ++i)
            {
                ...
                else if (!isValidShadowSlice)
                {
                    ...
                    // Therefore NPR RP try to keep the limit at sane levels when using uniform buffers.
        ...
        void SetupAdditionalLightsShadowReceiverConstants(CommandBuffer cmd, ref ShadowData shadowData, bool softShadows)
        {
            ...
            if (m_UseStructuredBuffer)
            {
                ...
                var shadowBuffer = CShaderData.instance.GetShadowDataBuffer(shadowLightsCount);
                shadowBuffer.SetData(shadowBufferData);

                var shadowIndicesMapBuffer = CShaderData.instance.GetShadowIndicesBuffer(m_AdditionalShadowCastingLightIndicesMap.Count);
