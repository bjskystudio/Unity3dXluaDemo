...
{
    /// <summary>
    /// Default renderer for NPR RP.
    /// This renderer is supported on all NPR RP supported platforms.
    /// It uses a classic forward rendering strategy with per-object light culling.
    /// </summary>
    public sealed class CNPRRenderer : AScriptableRenderer
    {
        const int k_DepthStencilBufferBits = 32;
        const string k_CreateCameraTextures = "Create Camera Texture";

        CColorGradingLutPass m_ColorGradingLutPass;
        CDepthOnlyPass m_DepthPrepass;
        CMainLightShadowCasterPass m_MainLightShadowCasterPass;
        CAdditionalLightsShadowCasterPass m_AdditionalLightsShadowCasterPass;
        ...
        CCopyDepthPass m_CopyDepthPass;
        CCopyColorPass m_CopyColorPass;
        CTransparentSettingsPass m_TransparentSettingsPass;
        CDrawObjectsPass m_RenderTransparentForwardPass;
        CInvokeOnRenderObjectCallbackPass m_OnRenderObjectCallbackPass;
        CCameraBlurProcessPass m_CameraBlurProcessPass;
        CCameraBloomPass m_CameraBloomPass;
        CPostProcessPass m_PostProcessPass;
        CPostProcessPass m_FinalPostProcessPass;
        CFinalBlitPass m_FinalBlitPass;
        CCapturePass m_CapturePass;
...
#if UNITY_EDITOR
        CSceneViewDepthCopyPass m_SceneViewDepthCopyPass;
#endif
        ...
        RenderTargetHandle m_OpaqueTransparentTexture;
        RenderTargetHandle m_AfterPostProcessColor;
        RenderTargetHandle m_ColorGradingLut;

        CLights m_ForwardLights;
        ...
        public CNPRRenderer(CNPRRendererData data) : base(data)
        {
            ...
            m_MainLightShadowCasterPass = new CMainLightShadowCasterPass(RenderPassEvent.BeforeRenderingShadows);
            m_AdditionalLightsShadowCasterPass = new CAdditionalLightsShadowCasterPass(RenderPassEvent.BeforeRenderingShadows);
            ...
            m_ColorGradingLutPass = new CColorGradingLutPass(RenderPassEvent.BeforeRenderingPrepasses, data.postProcessData);
            ...
            m_CopyDepthPass = new CCopyDepthPass(RenderPassEvent.AfterRenderingSkybox, m_CopyDepthMaterial);
            m_DrawSkyboxPass = new CDrawSkyboxPass(RenderPassEvent.BeforeRenderingSkybox);
            m_CopyColorPass = new CCopyColorPass(RenderPassEvent.AfterRenderingTransparents, m_SamplingMaterial);
            m_TransparentSettingsPass = new CTransparentSettingsPass(RenderPassEvent.BeforeRenderingTransparents, data.shadowTransparentReceive);
            ...
            m_OnRenderObjectCallbackPass = new CInvokeOnRenderObjectCallbackPass(RenderPassEvent.BeforeRenderingPostProcessing);
            m_CameraBlurProcessPass = new CCameraBlurProcessPass(RenderPassEvent.BeforeRenderingPostProcessing);
            m_CameraBloomPass = new CCameraBloomPass(RenderPassEvent.BeforeRenderingPostProcessing);
            m_PostProcessPass = new CPostProcessPass(RenderPassEvent.BeforeRenderingPostProcessing, data.postProcessData, m_BlitMaterial);
            m_FinalPostProcessPass = new CPostProcessPass(RenderPassEvent.AfterRendering + 1, data.postProcessData, m_BlitMaterial);
            m_CapturePass = new CCapturePass(RenderPassEvent.AfterRendering);
            m_FinalBlitPass = new CFinalBlitPass(RenderPassEvent.AfterRendering + 1, m_BlitMaterial);
...
#if UNITY_EDITOR
            m_SceneViewDepthCopyPass = new CSceneViewDepthCopyPass(RenderPassEvent.AfterRendering + 9, m_CopyDepthMaterial);
#endif
            ...
            m_OpaqueTransparentTexture.Init("_CameraOpaqueTransparentTexture");
            m_AfterPostProcessColor.Init("_AfterPostProcessTexture");
            m_ColorGradingLut.Init("_InternalGradingLut");
            m_ForwardLights = new CLights();
        ...
        public override void Setup(ScriptableRenderContext context, ref SRenderingData renderingData)
        {
            ...
            var postProcessFeatureSet = CNPRPipeline.asset.postProcessingFeatureSet;
            ...
            isRunningHololens = NPRPipeline.IsRunningHololens(camera);
#endif
            ...
            {
                m_DepthPrepass.Setup(cameraData.clearDepthForDepthOnlyPass, cameraTargetDescriptor, m_DepthTexture);
...
#pragma warning restore 0618
            ...
            if (renderingData.cameraData.requiresOpaqueTransparentTexture)
            {
                // TODO: Downsampling method should be store in the renderer instead of in the asset.
                // We need to migrate this data to renderer. For now, we query the method in the active asset.
                Downsampling downsamplingMethod = CNPRPipeline.asset.opaqueDownsampling;
                m_CopyColorPass.Setup(m_ActiveCameraColorAttachment.Identifier(), m_OpaqueTransparentTexture, downsamplingMethod);
            ...
            // 摄像机模糊操作
            if (cameraData.bBlurProcessEnabled)
            {
                m_CameraBlurProcessPass.Setup(m_ActiveCameraColorAttachment, CNPRPipeline.asset.cameraBlurProcessDownsampling, CNPRPipeline.asset.cameraBlurProcessScale);
                EnqueuePass(m_CameraBlurProcessPass);
            }

            // 摄像机辉光操作
            if (cameraData.bBloomProcessEnabled && CNPRPipeline.asset.cameraBloomIntensity > 0.0f && CNPRPipeline.asset.cameraBloomEnable)
            {
                m_CameraBloomPass.Setup(m_ActiveCameraColorAttachment, CNPRPipeline.asset.cameraBloomIntensity, CNPRPipeline.asset.cameraBloomThreshold, CNPRPipeline.asset.cameraBloomStepBlurSpread, CNPRPipeline.asset.cameraBloomColor, CNPRPipeline.asset.cameraBloomBlendFactor);
                EnqueuePass(m_CameraBloomPass);
            }
        ...
        /// <inheritdoc />
        public override void SetupLights(ScriptableRenderContext context, ref SRenderingData renderingData)
        {
            ...
            ref SCameraData cameraData)
        {
            ...
            bool isShadowCastingDisabled = !CNPRPipeline.asset.supportsMainLightShadows && !CNPRPipeline.asset.supportsAdditionalLightShadows;
        ...
        void CreateCameraRenderTarget(ScriptableRenderContext context, ref SCameraData cameraData)
        {
        ...
        bool RequiresIntermediateColorTexture(ref SRenderingData renderingData, RenderTextureDescriptor baseDescriptor)
        {
            ...
            bool requiresBlitForOffscreenCamera = cameraData.postProcessEnabled || cameraData.requiresOpaqueTransparentTexture || requiresExplicitMsaaResolve;
        ...
        bool CanCopyDepth(ref SCameraData cameraData)
        {
