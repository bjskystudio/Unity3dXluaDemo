...
namespace UnityEngine.Rendering.Universal
{
    ...
    /// </summary>
    [MovedFrom("UnityEngine.Rendering.LWRP")] public abstract class AScriptableRenderer : IDisposable
    {
        /// <summary>
        /// Configures the supported features for this renderer. When creating custom renderers
        /// for NPR Render Pipeline you can choose to opt-in or out for specific features.
        /// </summary>
        public class RenderingFeatures
        {
            ...
            /// <seealso cref="CAdditionalCameraData.cameraStack"/>
            /// </summary>
        ...
        void SetShaderTimeValues(float time, float deltaTime, float smoothDeltaTime, CommandBuffer cmd = null)
        {
            ...
            if (cmd == null)
            {
                Shader.SetGlobalVector(CNPRPipeline.PerFrameBuffer._Time, timeVector);
                Shader.SetGlobalVector(CNPRPipeline.PerFrameBuffer._SinTime, sinTimeVector);
                Shader.SetGlobalVector(CNPRPipeline.PerFrameBuffer._CosTime, cosTimeVector);
                Shader.SetGlobalVector(CNPRPipeline.PerFrameBuffer.unity_DeltaTime, deltaTimeVector);
                Shader.SetGlobalVector(CNPRPipeline.PerFrameBuffer._TimeParameters, timeParametersVector);
            }
            else
            {
                cmd.SetGlobalVector(CNPRPipeline.PerFrameBuffer._Time, timeVector);
                cmd.SetGlobalVector(CNPRPipeline.PerFrameBuffer._SinTime, sinTimeVector);
                cmd.SetGlobalVector(CNPRPipeline.PerFrameBuffer._CosTime, cosTimeVector);
                cmd.SetGlobalVector(CNPRPipeline.PerFrameBuffer.unity_DeltaTime, deltaTimeVector);
                cmd.SetGlobalVector(CNPRPipeline.PerFrameBuffer._TimeParameters, timeParametersVector);
            }
        }
        ...
        protected List<AScriptableRendererFeature> rendererFeatures
        {
            get => m_RendererFeatures;
        }

        protected List<AScriptableRenderPass> activeRenderPassQueue
        {
        ...
        List<AScriptableRenderPass> m_ActiveRenderPassQueue = new List<AScriptableRenderPass>(32);
        List<AScriptableRendererFeature> m_RendererFeatures = new List<AScriptableRendererFeature>(10);
        ...
        public AScriptableRenderer(AScriptableRendererData data)
        {
        ...
        /// <seealso cref="ScriptableRenderPass"/>
        /// <seealso cref="ScriptableRendererFeature"/>
        public abstract void Setup(ScriptableRenderContext context, ref SRenderingData renderingData);
        ...
        /// <param name="context">Use this render context to issue any draw commands during execution.</param>
        /// <param name="renderingData">Current render state information.</param>
        public virtual void SetupLights(ScriptableRenderContext context, ref SRenderingData renderingData)
        {
        }
        ...
        /// <param name="context">Use this render context to issue any draw commands during execution.</param>
        /// <param name="renderingData">Current render state information.</param>
        public void Execute(ScriptableRenderContext context, ref SRenderingData renderingData)
        {
        ...
        public void EnqueuePass(AScriptableRenderPass pass)
        {
        ...
        protected static ClearFlag GetCameraClearFlag(ref SCameraData cameraData)
        {
            ...
            // NPR RP doesn't support CameraClearFlags.DepthOnly and CameraClearFlags.Nothing.
        ...
        void SetCameraRenderState(CommandBuffer cmd, ref SCameraData cameraData)
        {
        ...
        void ExecuteBlock(int blockIndex, NativeArray<int> blockRanges,
            ScriptableRenderContext context, ref SRenderingData renderingData, int eyeIndex = 0, bool submit = false)
        {
        ...
        void ExecuteRenderPass(ScriptableRenderContext context, AScriptableRenderPass renderPass, ref SRenderingData renderingData, int eyeIndex)
        {
            ref SCameraData cameraData = ref renderingData.cameraData;
            Camera camera = cameraData.camera;
            ...
            if (CUtils.IsMRT(renderPass.colorAttachments))
            {
                ...
                int cameraColorTargetIndex = CUtils.IndexOf(renderPass.colorAttachments, m_CameraColorTarget);
                ...
                if (needCustomCameraColorClear)
                {
                    ...
                    {
                        uint otherTargetsCount = CUtils.CountDistinct(renderPass.colorAttachments, m_CameraColorTarget);
                ...
                if (!CUtils.SequenceEqual(renderPass.colorAttachments, m_ActiveColorAttachments) || renderPass.depthAttachment != m_ActiveDepthAttachment || finalClearFlag != ClearFlag.None)
                {
                    int lastValidRTindex = CUtils.LastValid(renderPass.colorAttachments);
        ...
        void EndXRRendering(ScriptableRenderContext context, in SRenderingData renderingData, int eyeIndex)
        {
        ...
        internal static void SortStable(List<AScriptableRenderPass> list)
        {
            int j;
            for (int i = 1; i < list.Count; ++i)
            {
                AScriptableRenderPass curr = list[i];
