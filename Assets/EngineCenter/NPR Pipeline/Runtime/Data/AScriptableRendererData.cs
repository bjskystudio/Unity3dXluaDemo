...
namespace UnityEngine.Rendering.Universal
{
    ...
    /// </summary>
    [MovedFrom("UnityEngine.Rendering.LWRP")] public abstract class AScriptableRendererData : ScriptableObject
    {
        ...
        protected abstract AScriptableRenderer Create();

        [SerializeField] public List<AScriptableRendererFeature> m_RendererFeatures = new List<AScriptableRendererFeature>(10);
        ...
        /// </summary>
        public List<AScriptableRendererFeature> rendererFeatures
        {
        ...
        internal AScriptableRenderer InternalCreateRenderer()
        {
            isInvalidated = false;
            return Create();
        }
            ...
            for (var i = 0; i < m_RendererFeatures?.Count; i++)
            {
                if (m_RendererFeatures[i] == null)
                {
                    if (mapValid && m_RendererFeatureMap[i] != 0)
                    {
                        ...
                        m_RendererFeatures[i] = (AScriptableRendererFeature)asset;
                    }
                    else
                    {
                        m_RendererFeatures[i] = (AScriptableRendererFeature)GetUnusedAsset(ref linkedIds, ref loadedAssets);
                    }
                }
                debugOutput += m_RendererFeatures[i] != null ? $"-{i}:Linked\n" : $"-{i}:Missing\n";
            }
            if(CNPRPipeline.asset.debugLevel != PipelineDebugLevel.Disabled)
